...
#bert_model, bert_vocab, bert_tokenizer = get_pretrained(ctxs, 'bert-base-en-cased')
bert_model, bert_vocab, bert_tokenizer = get_pretrained(ctxs, 'roberta-large-en-cased')
#bert_model, bert_vocab, bert_tokenizer = get_pretrained(ctxs, 'gpt2-117m-en-cased')

bert_scorer = MLMScorer(bert_model, bert_vocab, bert_tokenizer, ctxs)
#bert_scorer = LMScorer(bert_model, bert_vocab, bert_tokenizer, ctxs)
...
def sentence_score(listS):
	scores= bert_scorer.score_sentences([" ".join(listS).lower()])
	return scores[0]
