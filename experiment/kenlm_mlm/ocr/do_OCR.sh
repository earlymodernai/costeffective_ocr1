...
CURRENTDIR=$(pwd)
#TMP_DIR=$(mktemp -d)
TMP_DIR="/path/ocr/postprocess/tesseract/tmppng"
cd $TMP_DIR
...
#tesseract $f"_t.png" $f"_ocr" -l --dpi 300 eng
#mogrify -set units PixelsPerInch -density 200 $f
tesseract $f $f"_ocr" -l eng
cat $f"_ocr.txt" >> raw_output
done

python3 $SCRIPTDIR/fix_linebreaks.py raw_output > output
